...
!     --- Module for 2D Quadratic Elements (8 nodes, 9 int)  (quad2DT9.f90)
...
!     element characteristics		
  INTEGER nnod,edim,nq1D,etype,nq	
	PARAMETER (nnod=8)      
...
!	PARAMETER (nq=4)            ! nq1D^edim
	...
	PARAMETER (nq1D=3)          ! # guassian point integration (in 1D)
	PARAMETER (nq=9)            ! nq1D^edim
	
	
!     quadrature and shape functions (set 1x)		
	DOUBLE PRECISION qw(nq),dH(nnod,edim,nq)
	DOUBLE PRECISION qpt(edim,nq),HH(nq,nq),H(nnod,nq)
...
!___________________________________________________________________________
	...
	SUBROUTINE set_quad()
		...
		DOUBLE PRECISION P(2),W(2)
		P(1) = dsqrt(3.0d0/5.0d0)
		P(2) = 0.0d0
